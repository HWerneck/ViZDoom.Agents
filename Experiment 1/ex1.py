...
Original file is located at
    https://colab.research.google.com/drive/1t5Gwgf_08UQlbScrl0euBhYlEnRRrL6M
...
class DoomEnvironment (Env):
    ...
    def __init__(self,
        ...
        self.possible_actions = np.eye(3).tolist()
        self.action_space = spaces.Discrete(len(self.possible_actions))

        self.empty_frame = np.zeros((240, 320, 3), dtype=np.uint8)
        #vars = self.game.get_available_game_variables()
        #print('Game variables are ', vars)
        return
    ...
    def create_observation(self):
        ...
        #data
        #entities
        #knowledge_graph, scene_graph

        observation = screen
        #observation = {screen, data}
        #observation = {screen, data, entities}
        #observation = {screen, data, knowledge_graph, scene_graph}
        return observation

    def calculate_object_distance(self, doom_object):
        player_x = self.game.get_game_variable(GameVariable.POSITION_X)
        player_y = self.game.get_game_variable(GameVariable.POSITION_Y)
        distance = math.sqrt(math.pow((doom_object.position_x - player_x), 2) + math.pow((doom_object.position_y - player_y), 2))
        return distance
    ...
    def _get_label_buffer_frame(self):
        ...
        if (self.game.is_episode_finished()):
            ...
            frame = np.stack([labels_buffer] * 3, -1)
            frame[labels_buffer == 0] = [0, 0, 0]
            frame[labels_buffer == 1] = [64, 64, 64]
            frame[labels_buffer > 1] = [255, 0, 0]
            frame[labels_buffer == 255] = [255, 255, 255]
            return frame
...
def create_environment(scenario: str = 'basic', config: str = 'basic',
    ...
    path = '/content/drive/My Drive/ViZDoom/Agents/Mestrado/Basic/Ex1/'
    scenario += '.wad'
    config += '.cfg'
...
def play_env(env_args, agent_args, n_envs, timesteps, callbacks, log_name, eval_freq = None, init_func = None):
    ...
    csv_log_path = f'/content/drive/My Drive/ViZDoom/Agents/Mestrado/Basic/Ex1/Data/{log_name}'
    csv_eval_log_path = f'/content/drive/My Drive/ViZDoom/Agents/Mestrado/Basic/Ex1/Data/{log_name}_eval'

    #Create the environments
    env = create_vec_env(n_envs, **env_args)
    env = VecMonitor(env, csv_log_path, info_keywords=('kills', 'ammo', 'health', 'hit count'))

    #Build the agent
    agent = ppo.PPO(policies.ActorCriticCnnPolicy, env, tensorboard_log = '/content/drive/My Drive/ViZDoom/Agents/Mestrado/Basic/Ex1/logs/tensorboard', seed = 0, **agent_args)
    ...
    if eval_freq is not None:
        ...
        eval_env = VecMonitor(eval_env, csv_eval_log_path, info_keywords=('kills', 'ammo', 'health', 'hit count'))

        callbacks.append(EvalCallback(
                                  eval_env,
                                  n_eval_episodes = 10,
                                  eval_freq = eval_freq,
                                  log_path = f'/content/drive/My Drive/ViZDoom/Agents/Mestrado/Basic/Ex1/logs/evaluations/{log_name}',
                                  best_model_save_path = f'/content/drive/My Drive/ViZDoom/Agents/Mestrado/Basic/Ex1/logs/models/{log_name}'))
...
agent1 = play_env(env_args, agent_args, n_envs = 2, timesteps = 200000, callbacks = [], log_name = 'basic_test', eval_freq = 5000)

gif_file_path = '/content/drive/My Drive/ViZDoom/Agents/Mestrado/Basic/Ex1/GIFs/basic_agent1.gif'
agent1 = ppo.PPO.load('/content/drive/My Drive/ViZDoom/Agents/Mestrado/Basic/Ex1/logs/models/basic1/best_model.zip')
make_gif(agent = agent1, file_path = gif_file_path, gif_env_args = gif_env_args)

agent2 = play_env(env_args, agent_args, n_envs = 2, timesteps = 200000, callbacks = [], log_name = 'basic2', eval_freq = 5000)

agent3 = play_env(env_args, agent_args, n_envs = 2, timesteps = 200000, callbacks = [], log_name = 'basic3', eval_freq = 5000)

agent4 = play_env(env_args, agent_args, n_envs = 2, timesteps = 200000, callbacks = [], log_name = 'basic4', eval_freq = 5000)

agent5 = play_env(env_args, agent_args, n_envs = 2, timesteps = 200000, callbacks = [], log_name = 'basic5', eval_freq = 5000)

agent6 = play_env(env_args, agent_args, n_envs = 2, timesteps = 200000, callbacks = [], log_name = 'basic6', eval_freq = 5000)

agent7 = play_env(env_args, agent_args, n_envs = 2, timesteps = 200000, callbacks = [], log_name = 'basic7', eval_freq = 5000)

agent8 = play_env(env_args, agent_args, n_envs = 2, timesteps = 200000, callbacks = [], log_name = 'basic8', eval_freq = 5000)

agent9 = play_env(env_args, agent_args, n_envs = 2, timesteps = 200000, callbacks = [], log_name = 'basic9', eval_freq = 5000)

agent10 = play_env(env_args, agent_args, n_envs = 2, timesteps = 200000, callbacks = [], log_name = 'basic10', eval_freq = 5000)
